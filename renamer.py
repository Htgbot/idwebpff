...
# Function to rename images based on JSON data and save them to a new folder
def rename_images(json_file, image_folder, new_folder):
    # Load the JSON file
    ...
    for icon_name, item_id in data.items():
        ...
        if os.path.exists(current_image_path):
            ...
            if not os.path.exists(new_image_path):
                # Rename and move the image to the new folder
                os.rename(current_image_path, new_image_path)
                print(f"Renamed {current_image_name} to {new_image_name} and moved to {new_folder}")
            else:
                print(f"Cannot move {current_image_name} to {new_folder} because {new_image_name} already exists there.")
        else:
...
# Call the function to start renaming images and moving them to the new folder
rename_images(json_file, image_folder, new_folder)
